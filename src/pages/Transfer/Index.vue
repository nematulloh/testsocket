  ...
  <b-container fluid>
    <b-row>
      ...
      <b-col sm="3" md="3" class="my-1">
        <button class="btn btn-primary mybtn" @click="Href('/transfer/create')">
          {{ $t("Қўшиш") }}
        </button>
    ...
    >
      ...
      <template #cell(actions)="row">
        <router-link
          class="mybtn btn"
          :to="{ path: '/transfer/update/' + row.item.id }"
        >
...
export default {
  ...
  methods: {
    filterFunction,
    Update(record, index) {
      this.$router.push({ path: '/transfer/update/' + record.id});
    },
    Data() {
      let self = this;
      axios({

  ...
  <b-container fluid>
    <b-row>
      ...
      <b-col sm="3" md="3" class="my-1">
        <button class="btn btn-primary mybtn" @click="Href('/refund/create')">
          {{ $t("Қўшиш") }}
        </button>
    ...
    >
      ...
      <template #cell(actions)="row">
        <router-link
          class="mybtn btn"
          :to="{ path: '/refund/update/' + row.item.id }"
        >
...
export default {
  ...
  methods: {
    filterFunction,
    Update(record, index) {
      this.$router.push({ path: '/refund/update/' + record.id});
    },
    Data() {
      let self = this;
      axios({

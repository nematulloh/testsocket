<template>
  <div class="content">
    <form>
      <refundsuplier-form :datas="dataall"></refundsuplier-form>
...
<script>
import { RefundsuplierForm, Kontragent, Sklad, Firma, Paytype } from "@/pages";
export default {
  components: {
    RefundsuplierForm,
    Kontragent,
  ...
  beforeRouteLeave(to, from, next) {
    if (this.$store.state.saved == null) {
      ...
      if (answer) {
        this.$root.$refs.refundsuplier.Savedokument();
      } else {
        this.$store.state.saved = null;
        next();
      }
  ...
  methods: {
    Data() {
      let self = this;
      axios
        .get("api/v1/refund-supplier/id/" + self.$route.params.id, {})

  ...
  <b-container fluid>
    <b-row>
      ...
      <b-col sm="3" md="3" class="my-1">
        <button class="btn btn-primary mybtn" @click="Href('/initial/create')">
          {{ $t("Қўшиш") }}
        </button>
    ...
    >
      ...
      <template #cell(actions)="row">
        <router-link
          class="mybtn btn"
          :to="{ path: '/initial/update/' + row.item.id }"
        >
...
export default {
  ...
  methods: {
    filterFunction,
    Update(record, index) {
      this.$router.push({ path: '/initial/update/' + record.id});
    },
    Data() {
      let self = this;
      axios({
        method: "get",
        url: "api/v1/initial-balance"
    ...
    Trash(id) {
      ...
      self.$prompt(self.$t("Паролни киритинг")).then(text => {
          if (text == self.update_password) {
            axios({
            method: "delete",
            url: "api/v1/initial-balance/id/" + id
          }).then(function(response) {
            self.Data();
          });
          } else {

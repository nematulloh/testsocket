  ...
  <b-container fluid>
    ...
    >
      ...
      <template #cell(actions)="row">
        <router-link
          class="mybtn btn"
          :to="{ path: '/kassaorder/update/' + row.item.id }"
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "summa", label: "Cумма", sortable: true, class: "text-center" },
        { key: "actions", label: "" }
      ],
      totalRows: 1,
      currentPage: 1,
  ...
  methods: {
    filterFunction,
    Update(record, index) {
      this.$router.push({ path: '/kassaorder/update/' + record.id});
    },
    Data() {
      let self = this;
      axios({
        method: "get",
        url: "api/v1/kassa-order",
        headers: {

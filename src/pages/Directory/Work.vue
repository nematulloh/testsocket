      ...
      >
        <div class="row">
          <div style="display:-webkit-inline-box">
            <button type="button" class="mybtn btn" v-b-modal.modal-crmwork>
              ...
              <tr>
                ...
                <td>
                  <button
                    ...
                    class="atable"
                    @click="
                      datas.work_id = item.id;
                      $bvModal.hide('modal-crmworklist');
                    "
                  >
                    {{ item.name }}
                  </button>
...
export default {
  ...
  methods: {
    Trash(id) {
      ...
      if (answer) {
        let self = this;
        axios({ method: "delete", url: "api/v1/crm-work/id/" + id }).then(
          function(response) {
            self.List();
          }
        );
    ...
    List() {
      let self = this;
      axios.get("api/v1/crm-work").then(function(response) {
        self.list = response.data;
        self.$root.$emit("crm-workall", response.data);
      });
    },
    Selected(id) {
      this.$root.$emit("crm-workselected", id);
    },
    Update() {
      let self = this;
      axios({
        method: "patch",
        url: "api/v1/crm-work/id/" + self.models.id,
        data: self.models
      }).then(function(response) {
        self.List();
        self.models = { name: "" };
        self.$bvModal.hide("modal-crmworklist");
        self.$bvModal.hide("modal-crmworkset");
        self.$bvModal.show("modal-crmworklist");
      });
    },
    Shovedform() {
      let self = this;
    ...
    Hidecreate() {
      this.$bvModal.hide("modal-crmworklist");
      this.$bvModal.show("modal-crmworklist");
    },
    Create() {
      let self = this;
      axios({
        ...
        self.modelsnew = { name: "" };
        self.$bvModal.hide("modal-crmwork");
        self.$bvModal.show("modal-crmworklist");
        self.sett = "create";
      });
    }
  ...
  mounted: function() {
    this.$root.$emit("crm-workone", this.modelsnew);
    this.$root.$emit("crm-workselected", this.select);
    this.List();
  }
};
</script>

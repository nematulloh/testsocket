  ...
  <md-card>
    <div>
      <b-modal
        ...
        hide-footer
        size="xl"
        @shown="Shovedform('modal-countrylist')"
        @hidden="Hidecreate()"
      >
        <form>
          <div class="row">
      ...
      >
        <div class="row">
          <div style="display:-webkit-inline-box">
            <button type="button" class="mybtn btn" v-b-modal.modal-country>
              ...
              <tr>
                ...
                <td>
                  <button
                    ...
                    @click="
                      Selected(item.id);
                      $bvModal.hide('modal-countrylist');
                    "
                  >
                    {{ item.name }}
                  </button>
...
export default {
  ...
  methods: {
    ...
    List() {
      let self = this;
      axios.get("api/v1/country").then(function(response) {
        self.list = response.data;
        self.$root.$emit("countryall", response.data);
      });
    },
    Selected(id) {
      this.$root.$emit("countryselected", id);
    },
    Update() {
      let self = this;
      axios({
        method: "patch",
        url: "api/v1/country/id/" + self.models.id,
        data: self.models
      }).then(function(response) {
        self.List();
        self.models = { name: "" };
        self.$store.state.saved_modal = "updated";
        self.$bvModal.hide("modal-countryset");
      });
    },
    Hidecreate() {
      if (this.$store.state.saved_modal == "create") {
        ...
        if (answer) {
          ...
          this.modelsnew = { name: "" };
          this.$root.$emit("countryone", this.modelsnew);
        }
      ...
      this.$bvModal.show("modal-countrylist");
    },
    Create() {
      let self = this;
      axios({
        ...
        self.modelsnew = { name: "" };
        self.$bvModal.hide("modal-country");
        self.sett = "create";
        self.$root.$emit("countryone", self.modelsnew);
      });
    }
  },
  computed: {
  ...
  created() {
    this.$root.$emit("countryone", this.modelsnew);
    this.$root.$emit("countryselected", this.select);
    this.List();
  }
};
</script>

  ...
  <md-card>
    <div>
      <b-modal
        ...
        hide-footer
        size="xl"
        @shown="Shovedform('modal-brandlist')"
        @hidden="Hidecreate()"
      >
        <form>
          <div class="row">
    ...
    <div>
      <b-modal
        ...
        size="xl"
        @shown="Shoved"
        @hidden="Hide('modal-brandlist')"
      >
        <div class="row">
          <div style="display: -webkit-inline-box">
            <button type="button" class="mybtn btn" v-b-modal.modal-brand>
...
export default {
  ...
  methods: {
    ...
    List() {
      let self = this;
      axios.get("api/v1/brand").then(function(response) {
        self.list = response.data;
        self.$root.$emit("brandall", response.data);
      });
    },
    Hidecreate() {
      if (this.$store.state.saved_modal == "create") {
        ...
        if (answer) {
          ...
          this.modelsnew = { name: "" };
          this.$root.$emit("brandone", this.modelsnew);
        }
      ...
      this.$bvModal.show("modal-brandlist");
    },
    Selected(id) {
      this.$root.$emit("brandselected", id);
    },
    Update() {
      let self = this;
      axios({
        method: "patch",
        url: "api/v1/brand/id/" + self.models.id,
        data: self.models
      }).then(function(response) {
        self.List();
        self.models = { name: "" };
        self.$store.state.saved_modal = "updated";
        self.$bvModal.hide("modal-brandset");
      });
    },
    Create() {
      let self = this;
      axios({
        ...
        self.modelsnew = { name: "" };
        self.$bvModal.hide("modal-brand");
        self.sett = "create";
        self.$root.$emit("brandone", self.modelsnew);
      });
    }
  },
  computed: {
  ...
  created() {
    this.$root.$emit("brandone", this.modelsnew);
    this.$root.$emit("brandselected", this.select);
    this.List();
  }
};
</script>

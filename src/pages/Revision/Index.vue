  ...
  <b-container fluid>
    <b-row>
      ...
      <b-col sm="3" md="3" class="my-1">
        <button
          ...
          @click="Href('/revision/jamlash')"
        >
          {{ $t("Жамлаш") }}
        </button>
        <button class="btn btn-primary mybtn" @click="Href('/revision/create')">
          {{ $t("Қўшиш") }}
        </button>
      </b-col>
    </b-row>
    ...
    >
      ...
      <template #cell(actions)="row">
        <router-link
          class="mybtn btn"
          :to="{ path: '/revision/update/' + row.item.id }"
        >
...
export default {
  ...
  methods: {
    filterFunction,
    Update(record, index) {
      this.$router.push({ path: '/revision/update/' + record.id});
    },
    Data() {
      let self = this;
      axios({

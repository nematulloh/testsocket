  ...
  <div class="content">
    ...
    <div id="my-node">
      <h5>
        ...
        >
        <b style="float: right;">№{{ datas.daily_number }}</b>
      </h5>
      <ul v-for="(item, index) in datas.rasxod_table" :key="index">
        <li>
        ...
        <li style="text-align: right;">
          {{ item.count }}x{{ item.price }}={{ item.summa }}
        </li>
      </ul>
      <ul>
        <li>
      ...
      <ul>
        <li>
          <b>{{ $t("Скидка") }}:</b
          ><b style="float: right;">{{ datas.skidka_summa }}</b>
        </li>
      </ul>
      <ul>
        <li>
          <b>{{ $t("Тўлов") }}:</b
          ><b style="float: right;">{{
            parseFloat(datas.cash_summa) + parseFloat(datas.plastic_summa)
          }}</b>
        </li>
      </ul>
      <h5 style="text-align: center;">{{ mysklad.check_word }}</h5>

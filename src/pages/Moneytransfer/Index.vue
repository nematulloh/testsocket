<template>
  <b-container fluid>
    <b-row>
      ...
      <b-col sm="3" md="3" class="my-1">
        <button
          ...
          @click="Href('/money-transfer/create')"
        >
          {{ $t("Қўшиш") }}
        </button>
    ...
    >
      ...
      <template #cell(transfer_sklad)="row">
        <span v-if="row.item.transfer_sklad != null">
          {{ row.item.transfer_sklad.name }}
        </span>
      </template>
      <template #cell(total)="row">
        {{ row.item.total | numFormat }}
      </template>
      <template #cell(pay_type)="row">
        <span v-if="row.item.pay_type != null">
          {{ row.item.pay_type.name }}
        </span>
      </template>
      <template #cell(actions)="row">
        <router-link
          class="mybtn btn"
          :to="{ path: '/money-transfer/update/' + row.item.id }"
        >
...
export default {
  ...
  methods: {
    filterFunction,
    Update(record, index) {
      this.$router.push({ path: '/money-transfer/update/' + record.id});
    },
    Data() {
      let self = this;
      axios({
        method: "get",
        url: "api/v1/money-transfer"
    ...
    Trash(id) {
      ...
      self.$prompt(self.$t("Паролни киритинг")).then(text => {
          if (text == self.update_password) {
            axios({
            method: "delete",
            url: "api/v1/money-transfer/id/" + id
          }).then(function(response) {
            self.Data();
          });
          } else {
